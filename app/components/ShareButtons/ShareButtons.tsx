...
const CTA_TEXT = 'Visit the website for more details:';
const LINE_BREAK = '\n\n';

export default function ShareButtons({ url, title, description }: ShareButtonsProps) {
  const [copied, setCopied] = useState(false);

  // Construct share text: Intro + (SPACE) + URL + (SPACE) + Outro
  const shareText = `${INTRO_TEXT} ${LINE_BREAK} ${title} ${LINE_BREAK} ${CTA_TEXT} ${LINE_BREAK} ${url} ${LINE_BREAK} ${OUTRO_TEXT}`;

  // Handle Copy Link
  const handleCopyLink = async () => {
    try {
      // Copy full text: Intro + (SPACE) + URL + (SPACE) + Outro
  ...
  // Social media share links with new text structure
  const shareLinks = {
    instagram: `https://www.instagram.com/`, // Instagram doesn't support direct sharing, opens Instagram
    whatsapp: `https://wa.me/?text=${encodeShareData(shareText)}`,
    twitter: `https://twitter.com/intent/tweet?text=${encodeShareData(shareText)}`,
    linkedin: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeShareData(url)}`,
  };

  const handleSocialShare = (platform: 'instagram' | 'whatsapp' | 'twitter' | 'linkedin') => {
    if (platform === 'instagram') {
      // Instagram doesn't support direct URL sharing, so we'll copy the text and open Instagram
      handleCopyLink();
      // Open Instagram in a new tab (user can paste manually)
